...
public class TDManager : MonoBehaviour 
{
    ...
    private RaycastHit CheckPosition()
    {
        ...
        if (Physics.Raycast(ray, out hit, Mathf.Infinity))
        {
            ...
            tower1.transform.position = newPos;

            //Set color of "show" tower based on the spot being available
            if (hit.transform.tag == "Ground")
            {
                //tower1.GetComponent<Renderer>().material.color = Color.green;
            }
            else
            {
                //tower1.GetComponent<Renderer>().material.color = Color.red;
            }
        }
        else
        {
    ...
    private IEnumerator PlaceTowers()
    {
        ...
        {
            GameObject newTower = Instantiate(tower1, new Vector3(Mathf.RoundToInt(hit.point.x) - 0.5F, 0.3F, Mathf.RoundToInt(hit.point.z) + 0.5F), Quaternion.identity) as GameObject;
            towers.Add(newTower);
